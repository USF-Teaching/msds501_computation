...
def test_compute_line_total():
    order = {"quantity": 2, "unit_price": 5.00}
...
def test_top_one_product():
    summary = {
        "apple": {"total_revenue": 5.00},
        "banana": {"total_revenue": 10.00},
    }
    top = top_n_products(summary, 1)
    assert top[0][0] == "banana"


def test_apply_bulk_discount():
    orders = [{"product": "apple", "quantity": 5, "unit_price": 10.00}]
    discounted_orders = apply_bulk_discount(orders, 5, 0.10)
    assert discounted_orders[0]["unit_price"] == 9.00
