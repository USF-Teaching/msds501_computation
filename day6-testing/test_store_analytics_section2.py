"""
test_store_analytics.py

Starter file for the "write your own tests" exercise.

pytest and the module under test are already imported below, and there's
one fully-worked example test to show you the pattern. Everything after
that is up to you: add your own test functions (name them test_something)
that check store_analytics.py against its docstrings.

Run your tests from this folder with:
    pytest -v
"""

import pytest
from store_analytics import (
    parse_order_row,
    compute_line_total,
    summarize_by_product,
    top_n_products,
    apply_bulk_discount,
    loyalty_tier,
    load_orders_from_csv,
    write_top_products_report,
)


# --- Example test (already written for you) -------------------------------
# parameterize to test same function on multiple values!!
def test_parse_order_row_valid_row():
    row = ["1001", "Widget", "4", "9.99", "alice@example.com"]
    order = parse_order_row(row)
    assert order == {
        "order_id": "1001",
        "product": "widget",
        "quantity": 4,
        "unit_price": 9.99,
        "customer_email": "alice@example.com",
    }


# --- Your tests go below here ----------------------------------------------
def test_parse_order_row_more_than_five_fields_rasies():
    row = ["1001", "Widget", "4", "9.99", "alice@example.com", "extra field"]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_parse_order_row_less_than_five_fields_rasies():
    row = ["1001", "Widget", "4", "9.99"]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_parse_order_row_empty_order_id_rasies():
    row = ["", "Widget", "4", "9.99", "alice@example.com" ]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_parse_order_row_empty_product_rasies():
    row = ["1001", "", "4", "9.99", "alice@example.com" ]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_parse_order_row_quantity_decimal_rasies():
    row = ["1001", "Widget", "4.1", "9.99", "alice@example.com" ]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_parse_order_row_quantity_negative_rasies():
    row = ["1001", "Widget", "-1", "9.99", "alice@example.com" ]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_parse_order_row_quantity_non_numeric_rasies():
    row = ["1001", "Widget", "abc", "9.99", "alice@example.com" ]
    with pytest.raises(ValueError):
        parse_order_row(row)

def test_compute_line_total_valid_total_cost():
    order = {"quantity" : 4, "unit_price" : 9.99}
    total_cost = compute_line_total(order)
    assert total_cost == 39.96

def test_summarize_by_product_valid_summary():
    orders = [{
        "order_id": "1001",
        "product": "widget",
        "quantity": 4,
        "unit_price": 9.99,
        "customer_email": "alice@example.com"
    },
    {
        "order_id": "1001",
        "product": "widget",
        "quantity": 4,
        "unit_price": 9.99,
        "customer_email": "alice@example.com"
    }
    ]
    summary = summarize_by_product(orders)
    assert summary == {
            "widget": {
                "total_quantity": 8,
                "total_revenue": 79.92,   # rounded to 2 decimals
                "order_count": 2,
            }
        }

def test_summarize_by_product_empty_list():
    orders = []
    summary = summarize_by_product(orders)
    assert summary == {}

# def test_sort_key_valid_top_n_products():
#     item = 
#     summary = summarize_by_product(orders)
#     assert sort_key == ()